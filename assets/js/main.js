    ...
    if (acc_type_selected == "All Types") {
        for (var i = 0; i < locations.length; i++) {
            if (locations[i].prov == province_selected) {
                selectedData.push({
                    name: locations[i].name, lat: locations[i].lat, lng: locations[i].lng, address: locations[i].address, website: locations[i].website,
        ...
        for (var j = 0; j < locations.length; j++) {
            if ((locations[j].acc_type.indexOf(acc_type_selected) >= 0) && (locations[j].prov == province_selected)) {
                selectedData.push({
                    name: locations[j].name, lat: locations[j].lat, lng: locations[j].lng, address: locations[j].address, website: locations[j].website,
        ...
        for (var k = 0; k < locations.length; k++) {
            if ((locations[k].acc_type.indexOf(acc_type_selected) >= 0) && locations[k].county == county_selected) {
                selectedData.push({
                    name: locations[k].name, lat: locations[k].lat, lng: locations[k].lng, address: locations[k].address, website: locations[k].website,
